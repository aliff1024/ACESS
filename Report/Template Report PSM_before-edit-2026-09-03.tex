...
\begin{itemize}
    \item \textbf{1.4.8 Visual Presentation (AAA):} Line length is bounded by a \tcode{ch}-based reading measure so that it stays within the recommended range as the learner changes font size, rather than growing with the viewport.
...
\begin{itemize}
    ...
    \item \textbf{Traditional State Management vs. React Context:} Redux was considered for global state management but was deemed overly verbose for the accessibility settings use case. React Context with the \tcode{AccessibilityProvider} pattern was chosen for its simplicity \cite{react_docs}, native React integration, and minimal boilerplate when propagating CSS theme attributes across the component tree.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Next.js 16 \cite{nextjs_ssr}:} React framework with the App Router, providing server-side rendering, file-system routing and request-level route protection through its \tcode{proxy} entry point \cite{nextjs_middleware}.
...
\begin{itemize}
    ...
    \item \textbf{Concurrency:} The managed PostgreSQL instance uses connection pooling supplied by the platform. A realistic worst-case burst for this platform's scale should stay well within a one-second response time rather than degrading sharply. Chapter~6 verifies this target with real load tests against the local development stack rather than leaving it asserted, though not yet against the deployed environment or to an actual breaking point.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Input Handling:} Rich-text lesson content is sanitised with DOMPurify \cite{dompurify} before rendering, which is the platform's principal cross-site-scripting control. All database access goes through parameterised PostgREST or \tcode{pg} queries, so string-concatenated SQL does not occur. Field-level validation is performed in the form components and enforced a second time by \tcode{CHECK} constraints in the schema. A single shared schema-validation library is \emph{not} currently used, and adopting one is listed as future work in Section~\ref{sec:design-limitations}.
\end{itemize}
...
Course form & Title & Required, non-blank & Client + \tcode{NOT NULL} \\ \hline
Course form & Slug & Unique across courses & \tcode{UNIQUE} constraint \\ \hline
Course form & Difficulty & One of beginner / intermediate / advanced & \tcode{CHECK} constraint \\ \hline
Course form & Status & One of draft / pending\_review / published / archived & \tcode{CHECK} constraint \\ \hline
Lesson editor & Rich text & Sanitised before render, disallowed markup stripped & Client sanitiser \\ \hline
Lesson editor & Sequence order & Integer greater than zero & \tcode{CHECK} constraint \\ \hline
Lesson editor & Lesson layout & One of standard / focus / two\_column / wide / slideshow & \tcode{CHECK} constraint \\ \hline
Quiz builder & Pass threshold & Integer between 0 and 100 & \tcode{CHECK} constraint \\ \hline
...
Enrolment & Status & One of active / completed / dropped, completion not client-settable & \tcode{CHECK} + guard trigger \\ \hline
Contact form & Category & One of five permitted categories & \tcode{CHECK} constraint \\ \hline
...
The following tables constitute the physical schema of the ACESS PostgreSQL database as it currently stands: 36 base tables and 364 columns (370 including the six columns of the \tcode{quiz_options_scoped} view, which is described in Section~\ref{sec:phys-db}). They were generated directly from the live database catalogue rather than transcribed, so the attribute names, types, defaults and constraints in them are the ones the database actually holds. The \textbf{Constraint} column reports the key role, referential target, uniqueness and nullability of each attribute. \tcode{CHECK} constraints, which apply to a value rather than to a column's structure, are discussed with the physical design in Section~\ref{sec:phys-db}.
...
\Ensure Rows in \tcode{recommendations}, replacing any previous set for this learner
...
The answer key is protected on the way in as well as on the way out. Learner code never reads \tcode{quiz_options} directly. It reads the \tcode{quiz_options_scoped} view, which returns the correctness flag as \tcode{NULL} unless a database function confirms that the reader is staff or has already submitted an attempt for that quiz. The client therefore cannot obtain the answers before submitting, even by querying the API directly.
...
The physical schema in this report is not the one in the previous revision. The project's migration history had reached 44 files that could \emph{not} rebuild the database: none of them contained a \tcode{CREATE TABLE} for \tcode{users}, \tcode{courses}, \tcode{lessons}, \tcode{enrollments}, \tcode{quizzes} or \tcode{certificates}, every one assuming those tables already existed. The core schema existed only in an out-of-band dump, so a clean rebuild produced a broken database, and the history had drifted from reality: one migration dropped a table that was still present in the live project.
...
\begin{itemize}
    ...
    \item \textbf{Load testing did not find an actual breaking point.} Three k6 scenarios and a targeted concurrency regression test (Chapter~6) confirmed the endpoints a real user action calls hold their latency thresholds at a realistic worst-case concurrency for this platform's scale, but none was pushed until it actually failed, so this report can state where headroom exists without yet stating where the ceiling is.
...
\begin{itemize}
    \item \textbf{Functional testing}, split into an automated end-to-end suite that is the primary evidence for functional correctness, driving a real browser against the real running application, plus a small manual spot-check of tasks that automation cannot judge.
    \item \textbf{WCAG 2.2 expert review}, a findings table covering every criterion in Table~\ref{tab:wcag_map}, with the objective, code- and browser-verifiable findings filled in and the subjective judgement calls left to be completed by the signing reviewer.
...
Where a result in this chapter has not yet been supplied, it is marked \textbf{TODO} rather than filled in with a plausible-looking placeholder.
...
The automated suite, the load tests, and the manual/heuristic pass were all run against the \textbf{local development stack}, not the deployed application: a Next.js development server (\tcode{next dev}) on \tcode{localhost:3000}, connected to a local Supabase instance running in Docker (Postgres, PostgREST and GoTrue), seeded by the same \tcode{supabase/seed/run.ts} script described in Section~\ref{sec:impl-scm}. Every test exercises the actual rendered interface against a real, running database, not a substitute for one, but the results reported in this chapter describe the local-stack build and should not be read as a claim about the separately hosted application at \url{https://acess-tau.vercel.app} until the same suite is re-run there. Twelve seeded personas across the three roles (three administrators, three educators, six learners) served as test accounts, each already carrying a realistic history (part-finished courses, quiz attempts, issued certificates) so that tests exercise real, varied data rather than an empty account.

One documentation gap was found and corrected while confirming this: the reference the test accounts are documented under, \tcode{docs/SEED_CREDENTIALS.md}, still listed thirty generic \tcode{learnerN@test.com} accounts and an \tcode{admin@acess.edu} login from an earlier seed generation, none of which exist in the current database. Only the twelve named personas in \tcode{supabase/seed/personas.ts} are real, confirmed directly against the running database with \tcode{select email, role, full_name from users}. The document was rewritten to describe those twelve accounts only.
...
Automated end-to-end suite & 3 September 2026, with a same-day fix-and-reverify cycle & The twelve browser-driven scenarios in Table~\ref{tab:automated_suite}, run against the local development stack (Section~\ref{sec:test-env}). \\ \hline
Load and stress testing & 3 September 2026 & Three k6 scenarios plus a targeted concurrency regression test, described in Section~\ref{sec:load-testing}, run against the local development stack. \\ \hline
Manual spot-check & TODO & Ten task scenarios in Table~\ref{tab:manual_spotcheck}, expected to take under an hour to complete. \\ \hline
WCAG 2.2 expert review & TODO & The project author completes the subjective rows of Table~\ref{tab:wcag_reviewer}. \\ \hline
User acceptance testing & TODO & Six persona-based sessions, distributed and their responses collected. \\ \hline
\end{tabular}
\end{center}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Stress and load.} Whether the endpoints a real user action actually calls hold up under realistic concurrent traffic, run for real with k6 against the local development stack (Section~\ref{sec:load-testing}), not left as a stated limitation.
\end{itemize}
...
The suite is organised as one independent check per scenario, each one signing in as a real seeded persona through the real login form and then driving the same screens, clicks and form fields a person would use. Where the on-screen result alone cannot confirm that a write genuinely happened -- for example, that approving an application actually changed its stored status, not only its on-screen label -- the check also confirms the change directly in the underlying data, not from the interface alone.

Table~\ref{tab:automated_suite} sets out all nineteen scenarios together with their real, final result: what is being tested, the steps a tester follows, the result that would confirm a pass, and what actually happened. The suite was first run on 3 September 2026 and found four genuine functional defects, plus a fifth that only fully surfaced once the others were fixed; all were then fixed in the codebase and the affected scenarios were re-run to confirm the fix, the same day. A further two scenarios (LEARNER-08 through LEARNER-10, EDUCATOR-04 and EDUCATOR-05, ADMIN-02 and PUBLIC-02) were added after that first pass specifically to widen coverage beyond one happy-path case per feature: they exercise invalid or repeated actions, empty states, a boundary condition, and an unauthenticated-access check, across more of the learner profiles rather than one profile standing in for all of them. Table~\ref{tab:automated_suite}'s Actual Result and Status columns report the final, post-fix state throughout, not the first run. The defects themselves, with root cause and how each fix was verified, are described in full in Section~\ref{sec:test-defects} rather than compressed into this table's cells; the relevant rows below cross-reference the matching defect number.
...
\footnotesize
\begin{landscape}
\begin{longtable}{|>{\raggedright\arraybackslash\tabbreak}p{1.6cm}|>{\raggedright\arraybackslash\tabbreak}p{2.3cm}|>{\raggedright\arraybackslash\tabbreak}p{3.3cm}|>{\raggedright\arraybackslash\tabbreak}p{2.7cm}|>{\raggedright\arraybackslash\tabbreak}p{4.2cm}|>{\raggedright\arraybackslash\tabbreak}p{3.7cm}|>{\raggedright\arraybackslash\tabbreak}p{4.1cm}|>{\raggedright\arraybackslash\tabbreak}p{1.5cm}|}
\caption{Automated Suite Test Cases and Results}
\label{tab:automated_suite}\\
\hline
\textbf{ID} & \textbf{Role / Profile} & \textbf{Description} & \textbf{Test Data} & \textbf{Steps} & \textbf{Expected Result} & \textbf{Actual Result} & \textbf{Status} \\
\hline \hline
\endfirsthead
...
\endfoot
\endlastfoot
PUBLIC-01 & Anonymous visitor & Confirm the landing page is reachable without signing in, and check what happens when an anonymous visitor tries to reach the course catalogue. & No account signed in; the home page and the course catalogue link. & 1) Open the site's home page without logging in. 2) Confirm the page loads normally. 3) Try to open the course catalogue page. & The home page loads normally. The catalogue either shows a public preview of available courses, or sends the visitor to the login page, confirming which one it actually does. & The home page loaded correctly. Trying to open the course catalogue sent the visitor straight to the login page instead of showing a preview -- there is no public catalogue page at all. & Pass \\ \hline
PUBLIC-02 & Anonymous visitor & Confirm that trying to open a page meant only for administrators, without being signed in at all, is blocked and redirected to the login page. & No account signed in; the administrator dashboard page. & 1) With no account signed in, type the web address of the administrator dashboard directly into the browser. 2) See what page actually loads. & The visitor is sent to the login page instead of seeing the administrator dashboard. & The visitor was redirected straight to the login page, with a note that they would be returned to the dashboard after signing in. The administrator page itself was never shown. & Pass \\ \hline
LEARNER-01 & Learner (baseline) & Enrol in a course the learner is not already taking, using the real course catalogue. & haziq; the course ``Focus and Study Skills for ADHD Learners''. & 1) Log in as haziq. 2) Open the course's detail page. 3) Click the ``Enroll in Course'' button. 4) Check the learner's enrolment records afterward. & The ``Enroll in Course'' button disappears once clicked, and a new active enrolment appears for the learner. & Exactly as expected -- the enrolment was recorded and later removed again to leave the account as it was. (The course originally chosen for this check no longer existed in the system, so it was swapped for one that does.) & Pass \\ \hline
LEARNER-02 & Learner (ADHD preset) & Apply the ADHD accessibility preset, then complete a lesson that had not been finished yet. & amir; the lesson ``Capturing Notes That You Will Actually Reread''. & 1) Log in as amir. 2) Turn on the ADHD preset from the Accessibility Settings page and save it. 3) Open the lesson. 4) Read through the lesson, move to the last step, and click ``Complete Lesson''. & A learner using the ADHD preset can reach and use a ``Complete Lesson'' button, the same way a learner with no preset active can. & \textbf{First run: failed} -- there was no way to reach a ``Complete Lesson'' button at all while the ADHD preset was active (Defect 6). \textbf{Fixed and re-run: passed} -- the step-by-step navigation bar now appears correctly for ADHD learners, and the lesson was marked complete successfully. & Pass (fixed; Defect 6) \\ \hline
LEARNER-03 & Learner (Dyslexia preset) & Take a quiz attempt through the normal quiz screen. & mei; the quiz ``Fluency Check''. & 1) Log in as mei, whose profile already has the Dyslexia preset and simplified reading level turned on. 2) Start a quiz attempt. 3) Answer the first question. & The quiz keeps showing either the next question or feedback on the answer just given, rather than the screen going blank. & An ``Easy Read Mode'' banner appeared, the quiz was presented one question at a time rather than all at once, and answering the first question correctly advanced to the next step. (The lesson originally chosen for this check had no quiz attached at all, so it was swapped for one that does.) & Pass \\ \hline
LEARNER-04 & Learner & View a certificate already earned. & amir. & 1) Log in as amir. 2) Open the Achievements \& Certificates page. & At least one certificate is shown, with a reference code, the instructor's name and a completion date. & Two certificates were shown -- one issued automatically by the platform and one issued by an educator -- both with the correct details. Nothing on the account was changed by this check. & Pass \\ \hline
LEARNER-05 & Learner (baseline control) & Complete a lesson that had genuinely not been finished yet, with no accessibility preset active. & aisyah; the lesson ``Mapping the Structure of an Argument''. & 1) Log in as aisyah with no preset active. 2) Open the lesson and scroll to the end. 3) Click ``Complete Lesson''. 4) Check whether the lesson is now marked complete on the learner's progress page. & The button is visible and clickable, and the lesson is marked complete afterwards. & Exactly as expected. This is the plain, no-preset case that the ADHD, Dyslexia and Autism versions of the same check (LEARNER-02, 06 and 07) are compared against. & Pass \\ \hline
LEARNER-06 & Learner (Dyslexia preset) & Complete a lesson that had genuinely not been finished yet, under the Dyslexia preset. & mei; the lesson ``Dictation and Voice Typing'' (includes a video). & 1) Log in as mei with the Dyslexia preset active. 2) Confirm having watched the lesson's video. 3) Scroll to the end of the lesson. 4) Click ``Complete Lesson''. & The lesson is marked complete, the same as it would be with no preset active. & Exactly as expected, once the video was actually marked as watched first -- clicking ``Complete Lesson'' beforehand correctly refused to finish the lesson and explained what was still missing. The lesson text also appeared in the intended dyslexia-friendly font. & Pass \\ \hline
LEARNER-07 & Learner (Autism preset) & Confirm the ``I have watched this video'' button can actually be clicked under the Autism preset's step-by-step lesson layout. & priya; the lesson ``Beating the Starting Problem''. & 1) Log in as priya with the Autism preset active. 2) Find the ``I have watched this video'' button. 3) Click it. & Clicking the button registers the click and marks the video as watched. & \textbf{First run: failed} -- a floating checklist panel sat visually on top of the button and silently swallowed the click, so nothing happened (Defect 2). \textbf{Fixed and re-run: passed} -- the click now reaches the real button and registers correctly. & Pass (fixed; Defect 2) \\ \hline
LEARNER-08 & Learner (baseline) & Confirm a learner cannot accidentally enrol twice in a course they are already taking. & haziq; a course he is already enrolled in. & 1) Log in as haziq. 2) Revisit the detail page of a course he is already taking. & There is no ``Enroll in Course'' button to click again -- the page instead offers to continue the course. & Exactly as expected -- the page showed a ``Continue Learning'' option in place of an enrol button, so there was no way to trigger a second enrolment in the same course. & Pass \\ \hline
LEARNER-09 & Learner (no certificates yet) & Open the certificates page for a learner who has not earned any certificates yet. & haziq, whose account has not completed any course. & 1) Log in as haziq. 2) Open the Achievements \& Certificates page. & The page clearly explains that no certificates have been earned yet, rather than showing a blank or broken page. & Exactly as expected -- the page showed a clear ``No certificates yet'' message. & Pass \\ \hline
LEARNER-10 & Learner (ADHD preset, quiz attempts used up) & Open a quiz the learner has already attempted the maximum number of times allowed, and confirm the interface explains why no further attempt is possible. & amir, whose account has already used both of the two attempts allowed on the ``ADHD Study Skills Assessment'' quiz. & 1) Log in as amir. 2) Open the lesson containing that quiz. 3) Navigate through the lesson to reach the quiz section. & No ``Start Attempt'' button is offered, and the learner sees a message explaining that all allowed attempts have been used. & \textbf{First run: failed} -- no ``Start Attempt'' button appeared, which was correct, but no explanation appeared either, leaving what looked like an empty gap where the quiz should be (Defect 7). \textbf{Fixed and re-run: passed} -- the quiz section now clearly states that the maximum number of attempts has been reached. & Pass (fixed; Defect 7) \\ \hline
EDUCATOR-01 \& 02 & Educator & Create a new course through the course-creation wizard, then edit its accessibility focus. & farah; a newly created test course. & 1) Log in as farah. 2) Work through all four steps of the course-creation wizard and create the course. 3) Confirm it appears in her course list. 4) Open its Settings tab, set its Primary Accessibility Focus to ADHD, and save. & The new course appears in the course list, and its accessibility focus is saved as ADHD. & Exactly as expected, once the check was pointed at the exact course it had just created rather than searching for it again by name (an earlier version occasionally picked up a leftover course from an interrupted run instead of the new one). & Pass \\ \hline
EDUCATOR-03 & Educator & View the course analytics dashboard. & siti, who has five published courses. & 1) Log in as siti. 2) Open the Analytics page. & Real per-course figures are shown: numbers enrolled, active, at risk, average progress, completion rate and average quiz score. & All figures rendered correctly for every course. Nothing was changed by this check. & Pass \\ \hline
EDUCATOR-04 & Educator & Confirm the course-creation wizard will not let an educator continue past the first step without filling in the required title and description. & farah; the course-creation wizard with the title and description left blank. & 1) Log in as farah. 2) Open the course-creation wizard. 3) Without typing anything into the title or description, try to click ``Continue''. & The ``Continue'' button stays disabled until both fields are filled in. & Exactly as expected -- ``Continue'' stayed disabled while the fields were empty, and became clickable as soon as both were filled in. & Pass \\ \hline
EDUCATOR-05 & Educator & View the analytics for a course that has no learners enrolled yet. & farah; her course ``Introduction to Assistive Technology'', which has zero enrolments. & 1) Log in as farah. 2) Open the Analytics page and find that course's figures. & The course shows sensible zero values (zero enrolled, zero progress, and so on) rather than a broken or missing calculation. & Exactly as expected -- every figure for the course showed a clean zero, with no broken or missing values anywhere on the page. & Pass \\ \hline
ADMIN-01 & Administrator & Approve a pending application from someone applying to become an educator. & aliff; the application from ``Tan Chee Meng''. & 1) Log in as an administrator. 2) Open the pending application. 3) Click ``Approve'' and confirm the action. 4) Check the application's status afterward. & The application's status changes to approved, and a real educator account is created for the applicant. & First attempt sometimes failed silently, with the status not changing at all; retrying immediately afterward always worked correctly. Both underlying causes were found and fixed (Defect 5); one of them is a quirk of the local test environment rather than the application itself, and can still occasionally need one retry. & Pass (flaky; Defect 5) \\ \hline
ADMIN-02 & Administrator & Reject a pending educator application -- the opposite decision to ADMIN-01's approval. & aliff; a temporary test application created specifically for this check. & 1) Log in as an administrator. 2) Open the pending application. 3) Click ``Reject'' and confirm the action. 4) Check the application's status afterward. & The application's status changes to rejected. & Exactly as expected -- the status changed to rejected, and the temporary application was removed afterward. & Pass \\ \hline
A11Y-04 & Learner, any accessibility preset & Confirm that closing the lesson page's ``Incomplete Tasks'' pop-up leaves the rest of the page usable. & mei; the pop-up dismissed three different ways (its own close button, the Escape key, and clicking outside it). & 1) Try to complete a lesson before all of its requirements are met, which opens an ``Incomplete Tasks'' pop-up. 2) Close the pop-up, trying each of the three ways in turn. 3) After each one, try clicking something else on the page. & The pop-up closes cleanly every time, and the rest of the page stays fully clickable afterward. & \textbf{First run: failed} -- after closing the pop-up any way at all, the entire page became permanently unclickable for a mouse user, with no visible sign anything was wrong (Defect 1). \textbf{Fixed and re-run: passed} -- all three ways of closing the pop-up leave the page fully usable afterward. & Pass (fixed; Defect 1) \\ \hline
\end{longtable}
\end{landscape}
}
...
ACESS has no separate REST API layer of its own for the operations tested here: the application's server and client code calls Supabase PostgREST directly through \tcode{supabase-js}, confirmed by reading \tcode{src/lib/learner-api.ts}. Each load scenario below therefore calls the exact PostgREST endpoint, table, or remote procedure the real application calls for that action, under the same row-level-security and authentication model (the anon key for a public read, a real learner's own JSON Web Token for an authenticated write) rather than a synthetic stand-in. All three scenarios were run with k6 v2.2.0 against the local development stack (Section~\ref{sec:test-env}).

\textbf{Concurrent course-catalogue reads.} This calls \tcode{GET /rest/v1/courses} with the exact field list \tcode{fetchLearnerCourses()} selects, using the anon key, since the ``Public can view published courses'' policy carries no role restriction. ACESS is a small institutional deployment (twelve seeded accounts in this environment; the seed documentation describes a class-sized cohort), so 50 virtual users approximates an entire class plus staff opening the catalogue at the same moment, a realistic worst-case burst for this table rather than steady background traffic. Ramping from 0 to 50 users over 30 seconds, holding for 30 seconds and ramping down over 10 seconds gives the connection pool time to reach a steady state before percentiles are read. The result: 2{,}018 requests, a 0.00\% error rate, and a 95th-percentile response time of 6.99\,ms against an 800\,ms threshold. No degradation was observed at this load; the endpoint has clear headroom beyond what a class-sized user base would generate.

\textbf{Concurrent enrolments.} This calls \tcode{POST /rest/v1/enrollments} then \tcode{DELETE /rest/v1/enrollments}, each with the calling learner's own real token, exercising the actual ``Users can enrol themselves'' row-level-security policy per request rather than a service-role bypass. A single insert cannot sustain a duration test, since the \tcode{(user_id, course_id)} unique constraint blocks a repeat, so each virtual user alternates enrol then drop, the same real cycle LEARNER-01's own teardown performs. Forty virtual users, one per real throwaway learner account created for the test, for 30 seconds, models a whole cohort enrolling in the same newly announced course inside the same half-minute. Result: 2{,}444 requests, 0.00\% errors, 95th-percentile response time 65.86\,ms against a 1{,}000\,ms threshold. Every insert and every delete succeeded; no unique-constraint or policy-denial error surfaced under 40 concurrent writers.

\textbf{Concurrent quiz submissions.} This calls the \tcode{submit_quiz_attempt} remote procedure, the only path that can write a quiz result: \tcode{quiz_attempts} carries no insert policy at all, so grading happens exclusively inside this \tcode{SECURITY DEFINER} function, which verifies enrolment, enforces the attempt limit, and grades against the stored answer key server-side, closed off from a direct client write after an earlier forged-score incident recorded in the function's own source comment. The same 40 accounts and 30-second window as the enrolment scenario were used, for the same reason: a class submitting the same timed quiz at once is the realistic spike shape. The quiz used, ``Fluency Check'', has no attempt limit, the one quiz in the seed data able to absorb a sustained duration-based load without later attempts being rejected once an earlier one used up the limit. \textbf{The first run of this scenario surfaced a real concurrency defect}: of 1{,}292 requests, two failed with a raw \tcode{409} \tcode{"duplicate key value violates unique constraint"} error, and reading \tcode{submit_quiz_attempt}'s own source showed why: the function computed the next \tcode{attempt_number} with a plain \tcode{SELECT COALESCE(max(attempt_number), 0) + 1} and then inserted, with no lock between the read and the write, a genuine race for two truly concurrent submissions against the same enrolment and quiz. This is examined in full, with the fix and its verification, as Defect 4 in Section~\ref{sec:test-defects}. Once fixed, an identical re-run recorded 1{,}302 requests with a 0.00\% error rate, 95th-percentile response time 86.06\,ms against a 1{,}200\,ms threshold, and a dedicated concurrency regression script (described alongside Defect 4) confirmed the underlying race itself is closed, not merely absent from one run by chance.

Table~\ref{tab:load_results} summarises all three scenarios' final, post-fix results together. None of the three was pushed to an actual breaking point: all three held their latency thresholds with substantial headroom at the tested concurrency, which is evidence the endpoints can absorb a realistic burst for a deployment of this size, not evidence of where the ceiling actually is. Finding that ceiling, and testing sustained load rather than a single 30-second burst, remains future work (Section~\ref{sec:future-works}).

\begin{table}[H]
\caption{Load Test Results (post-fix)}
\label{tab:load_results}
\vspace{-4mm}
\begin{center}
\renewcommand{\arraystretch}{1.2}
\footnotesize
\begin{tabular}{|>{\raggedright\arraybackslash}p{3.3cm}|>{\raggedright\arraybackslash}p{2.0cm}|>{\raggedright\arraybackslash}p{1.9cm}|>{\raggedright\arraybackslash}p{2.0cm}|>{\raggedright\arraybackslash}p{2.9cm}|}
\hline
\textbf{Scenario} & \textbf{Virtual users} & \textbf{Duration} & \textbf{p95 latency} & \textbf{Error rate / outcome} \\
\hline \hline
Catalogue reads & 50 & 30s hold (70s total) & 6.99\,ms (target $<$800\,ms) & 0.00\% (0 / 2{,}018) \\ \hline
Concurrent enrolments & 40 & 30s & 65.86\,ms (target $<$1{,}000\,ms) & 0.00\% (0 / 2{,}444) \\ \hline
Concurrent quiz submissions & 40 & 30s & 86.06\,ms (target $<$1{,}200\,ms) & 0.00\% (0 / 1{,}302), down from 0.15\% before Defect 4 was fixed \\ \hline
Same-enrolment race regression & 1 (8 simultaneous requests) & one batch & avg 334\,ms & 0.00\% (0 / 8); 8 of 8 attempt numbers distinct, no duplicates \\ \hline
\end{tabular}
\end{center}
\end{table}
...
Table~\ref{tab:manual_spotcheck} lists ten tasks that need a person's judgement rather than an assertion, tagged to the learner profile each exercises. Actual Result and Status are left blank, to be completed once this spot-check is carried out. The tasks were scaffolded from real, already-implemented features described in Chapter~4, not performed or scored here.
...
MSC-08 & Autism & With the Autism preset active (\tcode{animation_level=none}), move through several pages of the platform. Is the experience genuinely free of distracting motion, or do any animations or transitions still slip through? & & \\ \hline
...
\endfoot
\endlastfoot
1.1.1 Non-text Content (A) & Partial & Rule confirmed in \tcode{accessibility-audit.ts}. Live scan of all 29 published lessons: 3 images total, 0 missing the \tcode{alt} attribute, 1 with \tcode{alt=""} (empty). Confirm whether that image is genuinely decorative: if not, this is a live content gap rather than a code defect. TODO: reviewer judgement. \\ \hline
...
1.4.8 Visual Presentation (AAA) & \textbf{Fail (fixed)} & Reading-measure values documented in Table~\ref{tab:preset_effects} were confirmed respected on a live rendered lesson. A separate, real conflict was found and fixed in the same theming layer this criterion covers: \tcode{applyReadingLevelDefaults()} auto-merged a legacy \tcode{preferred_theme: 'high_contrast'} value into any learner whose \tcode{preferred_reading_level} is \tcode{'simplified'} or \tcode{'basic'} (mei's Dyslexia persona among them) on every profile load, overriding the Dyslexia preset's own \tcode{background_tint}/\tcode{font_family} choice (cream, OpenDyslexic) with a high-contrast theme neither the preset nor Table~\ref{tab:preset_effects} describes -- two independent theming mechanisms fighting over the same learner. The conflicting override was removed; confirmed live afterwards that \tcode{data-theme} resolves to \tcode{light} and the cream/OpenDyslexic presentation applies uncontested. Full detail in Defect 3, Section~\ref{sec:test-defects}. \\ \hline
...
2.2.4 Interruptions (AAA) & Partial & Confirmed live in a real browser: distraction-free mode does remove navigation chrome during a lesson as intended, but it also removes the sidebar across the \emph{entire} application once the ADHD preset is active, not only inside a lesson as Table~\ref{tab:wcag_map} states, which left no way back to Accessibility Settings without resetting the account directly during this same testing pass. Separately, a best-effort heuristic read of whether the reduced-chrome view actually \emph{feels} calmer (Table~\ref{tab:manual_spotcheck}, MSC-04): objectively, it removes a measurable amount of simultaneously visible interface (sidebar, notification chrome, the other lesson phases) and narrows the view to one task at a time with an explicit exit control, a structural reduction, not a cosmetic one. Whether that reduction is \emph{felt} as calmer rather than disorienting is a genuinely subjective, embodied judgement a structural read cannot settle on its own; that verdict is left to MSC-04's own tester rather than asserted here. \\ \hline
2.2.6 Timeouts (AAA) & Pass & \tcode{SessionTimeout.tsx} confirmed: 15-minute inactivity timeout (\tcode{INACTIVITY_TIMEOUT_MS = 15 * 60 * 1000}), 30-second warning (\tcode{WARNING_BEFORE_MS = 30 * 1000}), matching the documented behaviour exactly. \\ \hline
2.3.3 Animation (AAA) & \textbf{Partial} & \tcode{html[data-animation-level="none"]} in \tcode{globals.css} sets \tcode{animation-duration}/\tcode{transition-duration} to \tcode{0.01ms} with \tcode{!important}, comprehensive for native CSS animations and transitions (confirmed it correctly suppresses the Tailwind \tcode{animate-ping} confetti effect on the lesson-completion celebration). It has no effect on Framer Motion, which does not animate through the CSS \tcode{transition}/\tcode{animation} properties this rule targets: \tcode{LessonViewPage.tsx}'s completion-celebration \tcode{motion.div} carries a hardcoded \tcode{transition={{ duration: 0.5 }}} with no reference to \tcode{settings.animation_level} anywhere in it, even though the sibling \tcode{CelebrationAnimation} component two lines below it does receive and respect that same setting. Native CSS keyframe effects are correctly suppressed under \tcode{animation_level='none'}; Framer-Motion-driven fades and slides are not, and still animate at their hardcoded duration regardless of the setting. Not a full component-by-component sweep, but a specific, confirmed instance of the gap rather than an unconfirmed suspicion. \\ \hline
...
2.5.8 Target Size (Minimum) (AA) & \textbf{Fail (fixed)} & Every interactive control checked meets the 24$\times$24\,px minimum on its own. The automated suite found a real target-size violation that has nothing to do with any single control's own dimensions: the Autism preset's floating checklist sidebar (\tcode{StepByStepGuidance.tsx}) is positioned with a fixed, unconditional \tcode{z-index} that sits above the in-lesson primary action bar wherever the two overlap, so the bottom portion of the ``Complete Lesson'' button -- itself sized correctly -- fell under the sidebar's hit area instead of the button's, and absorbed the click. Confirmed live: an identical click at the same on-screen position landed on the sidebar, not the button, for an Autism-preset learner. Fixed by giving the checklist sidebar a lower \tcode{z-index} than the action bar so overlapping controls resolve clicks to the visible top-most element instead of a fixed stacking order; confirmed live afterwards that the same click now reaches the button. Full detail in Defect 2, Section~\ref{sec:test-defects}. \\ \hline
3.1.5 Reading Level (AAA) & Partial & Flesch--Kincaid computation confirmed present in \tcode{accessibility-audit.ts}. Separately, the automated suite found that the quiz component a learner with \tcode{preferred_reading_level='simplified'} is routed into (mei's Dyslexia persona) shows raw, untranslated interface strings (\tcode{quiz.tryAgain}, \tcode{quiz.reviewLesson}, \tcode{quiz.questionTooltip}) instead of plain-language text on a wrong answer, the opposite of what a simplified-reading learner needs. TODO: reviewer judgement on whether a specific lesson's reported grade level matches your own read of its difficulty. \\ \hline
3.2.3 Consistent Navigation (AA) & TODO & Requires comparing the rendered shell across multiple pages and roles live. \\ \hline
3.2.5 Change on Request (AAA) & Partial & \tcode{PresetDetailsDialog.tsx} confirmed to render a before/after diff prior to applying a preset. Several embedded video/H5P \tcode{iframe} elements (\tcode{H5PViewer.tsx}, \tcode{LessonRenderer.tsx}, \tcode{AdminCourseDetail.tsx}, \tcode{CourseAssets.tsx}, \tcode{LessonEditor.tsx}) include \tcode{autoplay} in their permissions-policy \tcode{allow} attribute. None of the checked embed URLs pass an \tcode{autoplay} query parameter, so nothing appears to auto-start from the platform's side, but confirm this live since the permission is granted. \\ \hline
4.1.2 Name, Role, Value (A) & \textbf{Partial (since fixed)} & \tcode{Sidebar.tsx} confirmed to use real \tcode{<Link>} elements with \tcode{aria-current}, and icon-only controls carry \tcode{aria-label}/\tcode{aria-pressed} (this was found broken and fixed during earlier testing). Modal dialogs use Radix UI's \tcode{Dialog} primitive, which provides a maintained, independently tested focus-trap and Escape-to-close implementation rather than a custom one. Separately, the automated suite found that a lesson had no lesson-completion control of any role reachable under the ADHD preset at all (Table~\ref{tab:automated_suite}, LEARNER-02) -- not a naming or role defect on an existing control but the control's total absence, which is a functional gap this criterion cannot fully capture on its own. That absence has since been fixed and reverified (Defect 6, Section~\ref{sec:test-defects}); the control is now reachable and correctly named. \\ \hline
...
The suite was run on 3 September 2026 against the local development stack (Section~\ref{sec:test-env}), driving a real browser. Of the nineteen scenarios in Table~\ref{tab:automated_suite}, eighteen pass cleanly and one (ADMIN-01) passes reliably but is flaky on its first attempt for a documented environment cause rather than an application defect; none fail. That is a final, post-fix count: the first pass against the original twelve scenarios found three genuine functional defects (LEARNER-02, LEARNER-07, and A11Y-04, the last of which is not itself one of the original twelve but a defect the same run surfaced and which blocked other scenarios' teardown), and widening the suite afterward with seven further scenarios aimed specifically at invalid actions, empty states and a boundary condition turned up one more (LEARNER-10). Together with one defect found by the load tests and two by the manual/heuristic inspection, that makes seven in total, none of them visible to a database-only check, because every one is a failure of what actually renders or of a genuine race between concurrent requests rather than of stored data. All seven are written up in full, with root cause and how each fix was verified, immediately below.

\subsubsection{Defects Found and Fixed}
\label{sec:test-defects}

Seven real defects were found during this testing pass -- four by the automated suite, one by the load tests, and two by the manual/heuristic inspection -- and all seven were fixed and reverified the same day, 3 September 2026. Each is written up below in the order it was fixed: what was actually wrong, its root cause traced in the source, the fix applied, and how that fix was confirmed.

\textbf{Defect 1 -- stuck \tcode{pointer-events: none} after closing the ``Incomplete Tasks'' popup.} Closing that popup (a Radix \tcode{Dialog}, \tcode{LessonViewPage.tsx:2968}) left the entire page permanently unclickable for a mouse user: \tcode{document.body} kept \tcode{style="pointer-events: none"} even several seconds later, confirmed reproduced twice. The root cause traced directly into \tcode{node_modules/@radix-ui/react-dismissable-layer}: it tracks how many open layers currently want outside-pointer-events disabled in a single \emph{module-level} \tcode{Set} shared by every \tcode{Dialog}/\tcode{AlertDialog}/\tcode{Popover} in the whole application, because this codebase never wraps its component tree in Radix's own \tcode{DismissableLayerContext} provider. \tcode{LessonViewPage.tsx} renders five sibling \tcode{Dialog} roots at once (resources, discussion, AI assistant, asset preview, the completion checklist); when the checklist dialog's layer registers and unregisters against that shared count, the cleanup effect's own check that the count has returned to zero can simply never become true again, so the body-style restore it guards never runs. Rather than patch a third-party library's internals, a small watchdog component (\tcode{src/components/ui/dialog-pointer-events-guard.tsx}, mounted once in \tcode{src/app/providers.tsx}) was added: a \tcode{MutationObserver} plus an Escape/pointerdown recheck and a one-second safety-net interval clear \tcode{document.body}'s stray \tcode{pointer-events: none} the moment no dialog is actually open, so it can only ever remove a lock nothing still needs. Verified by a new automated check that triggers and dismisses the popup three separate ways (its own button, Escape, and clicking outside) and confirms \tcode{pointer-events} returns to \tcode{auto} and a real follow-up click succeeds every time; passed on three independent full-suite runs after the fix.

\textbf{Defect 2 -- Autism preset: the checklist sidebar intercepts clicks meant for the lesson controls beneath it.} Under the Autism preset's step-by-step layout, \tcode{document.elementFromPoint()} at the ``I have watched this video'' button's own on-screen coordinates returned the floating \tcode{StepByStepGuidance} checklist sidebar, not the button -- confirmed identically on two independent automated runs and once by hand. The sidebar (\tcode{LessonViewPage.tsx:1751--1753}) is positioned \tcode{fixed; right-6; top-44; w-72} from the \tcode{xl} breakpoint (1280\,px) up, on the assumption there is always unused margin beside the reading column at that width; at a typical \tcode{xl} viewport the centred content column's own \tcode{max-width} leaves only a few pixels more than the sidebar's own width-plus-gap needs, so the two overlap. Fixed in two parts: the breakpoint was raised to \tcode{2xl} (1536\,px), and the sidebar wrapper was made \tcode{pointer-events-none} by default (\tcode{VisualSchedule} has no interactive elements of its own to lose), with only \tcode{StepByStepGuidance}'s own card opting back into \tcode{pointer-events-auto} for its real Previous/Next/Exit/Complete controls -- so even a viewport this was not specifically tuned for cannot reproduce the click-swallow. Verified by the automated check for this scenario, deliberately run at 1920$\times$1080 (well past \tcode{2xl}, so the sidebar is still present) to confirm \tcode{elementFromPoint} now returns the button itself and a real, unforced click succeeds; passed cleanly on the final run.

\textbf{Defect 3 -- Dyslexia preset: two independent theming systems fighting over the same learner.} A Dyslexia-profile learner's page carried \tcode{data-theme="high_contrast"} on \tcode{<html>} at the same time as \tcode{background_tint: "cream"} and \tcode{font_family: "opendyslexic"} from the newer, more granular preset fields -- two theming mechanisms both active and visually contradicting each other. Root cause: \tcode{applyReadingLevelDefaults()} auto-merges \tcode{EASY_READ_PRESETS} (\tcode{accessibility-utils.ts:41--50}) into a learner's settings on every profile load whenever \tcode{preferred_reading_level} is \tcode{'simplified'} or \tcode{'basic'}, and that legacy preset hard-coded \tcode{preferred_theme: 'high_contrast'}, silently overriding the newer preset's own theme choice. Fixed by removing \tcode{preferred_theme} from \tcode{EASY_READ_PRESETS} entirely; Easy Read's other behaviour (larger font, looser spacing, the dyslexia-friendly font, simplified UI) is unchanged, only the conflicting theme override is gone. Verified live: logging in fresh as the Dyslexia persona now resolves \tcode{data-theme} to \tcode{"light"}, \tcode{preferred_theme} no longer appears in the merged settings object at all, and the cream background and OpenDyslexic font still apply correctly; the existing test covering this persona's font continues to pass.

\textbf{Defect 4 -- quiz attempt-number race condition under concurrent submission.} \tcode{submit_quiz_attempt()}, the only path that can write a quiz attempt (\tcode{quiz_attempts} has no \tcode{INSERT} policy of its own), computed the next \tcode{attempt_number} with \tcode{SELECT COALESCE(max(attempt_number), 0) + 1} and then inserted, with no lock between that read and the write -- a genuine time-of-check-to-time-of-use race for two truly concurrent submissions against the same enrolment and quiz. This was first flagged as a code-reading finding during the load test (Section~\ref{sec:load-testing}), then reproduced on demand: temporarily widening the unlocked function with an artificial delay and firing five simultaneous requests via a dedicated k6 script (\tcode{http.batch()}, which sends every request in a batch at once) produced two raw \tcode{409} \tcode{"duplicate key value violates unique constraint"} errors out of five, with the three winners' attempt numbers showing the collision directly. Fixed by a new migration adding \tcode{PERFORM pg_advisory_xact_lock(...)}, keyed on the enrolment and quiz id, immediately after resolving the enrolment and before computing the attempt number -- serialising concurrent calls for that specific enrolment-and-quiz pair without affecting any other pair. Verified by re-running the identical concurrent script against the real, unmodified fixed migration: five of five requests succeeded with attempt numbers \tcode{[1,2,3,4,5]}, no duplicates and no gaps; re-run again at eight concurrent requests with the same clean result.

\textbf{Defect 5 -- ADMIN-01 flakiness, two distinct causes.} Approving a pending instructor application failed silently on the first attempt in three of four full-suite runs on the day this was found (no error, no database write), always succeeding on an immediate retry. Two separate real causes were found, not one. \emph{Cause A:} \tcode{InstructorApplications.tsx} rendered the application-details dialog and the confirmation dialog as two separate Radix \tcode{Dialog} roots, so clicking ``Approve'' closed one dialog (a Radix layer unmounting) at the exact instant the confirmation dialog opened (a second layer mounting) -- the same category of shared-module-state race as Defect 1, and it manifested as a confirmed click producing no write. Fixed by collapsing all three dialog views (details, confirm-approve, confirm-reject) into a single \tcode{Dialog} whose content switches on a state variable, so only one Radix open/close transition is ever in flight. \emph{Cause B, found while investigating Cause A:} server logs showed the approval API route recompiling on demand (a \tcode{next dev} characteristic, since dev-mode compiles each route on first hit rather than ahead of time as a production build would) at the same moment a request body arrived, corrupting the body stream before the handler could read it and throwing \tcode{SyntaxError: Unexpected end of JSON input}. This is an artifact of the local development server rather than application logic, and cannot be fully eliminated in application code short of pre-warming the route; mitigated by sending one harmless warm-up request before test runs, and can still occasionally cost one retry in this environment. Verified by running the approval test in isolation three times after both fixes (three of three clean passes, no retries), and confirmed again on every subsequent full-suite run.

\textbf{Defect 6 -- ADHD focus mode: the real lesson-completion control was permanently unreachable.} Under the ADHD preset, no lesson-completion control of any kind could be reached through the interface for a lesson without a summary activity: ``Complete Lesson'' appeared only as inert checklist text, never as a clickable element. The proximate cause was that the real button is gated on \tcode{currentFocusId === 'summary'}, and ADHD forces \tcode{effectiveFocusMode} true -- but that alone only explained what was hidden, not why the state that reveals it could never be reached. The actual root cause sat one layer deeper: \tcode{focusStep} (the index driving \tcode{currentFocusId} through Video/Content/Activities/Summary) is changed only by the ``Focus Mode Slide Navigation'' bar's own Prev/Next arrows and step pills, and that bar's visibility used to be gated on the raw, manually-toggled \tcode{focusMode} flag rather than on \tcode{effectiveFocusMode} -- the condition every content section it controls, including the completion button itself, actually checks. ADHD sets \tcode{effectiveFocusMode} true through a completely different branch, without ever touching the raw flag, and ADHD's \tcode{simplifiedMode} is also always true, which independently hides the one manual control that could otherwise have set the raw flag. Put together, the one door able to advance \tcode{focusStep} could never render for an ADHD learner, \tcode{focusStep} stayed permanently at 0 for the whole lesson, \tcode{currentFocusId} could never become \tcode{'summary'}, and the real completion button was unreachable -- not because it was deliberately withheld, but because the one door to it was bolted shut by an unrelated flag. Fixed by keying that navigation bar's visibility on \tcode{effectiveFocusMode} instead of the raw flag, the same condition every section it controls already uses, so it now renders whenever focus mode is genuinely active regardless of how it was activated. Verified both by hand (logging in as the ADHD persona, confirming the navigation bar now renders where it never did, scrolling through the Content step, advancing to Summary via the step pills, clicking Complete Lesson, and confirming \tcode{lesson_progress.is_completed} became \tcode{true} directly in the database) and by an automated test rewritten to drive the same real navigation bar end to end; three of three clean passes in isolation, then included in a full twelve-test suite run that itself passed eleven of twelve cleanly with only the already-documented ADMIN-01 flake, unrelated to this fix, needing its one retry. Nothing else regressed.

\textbf{Defect 7 -- an exhausted quiz shows a blank card instead of explaining why.} Once the suite was widened to check a quiz attempted the maximum number of times allowed (LEARNER-10), a genuine defect turned up: the quiz section rendered its heading and question count as normal, then nothing else at all -- no ``Start Attempt'' button (correctly, since the limit really had been reached), but no explanation either, just an empty card where the result should have appeared. No console or network error accompanied it; the request that checks whether another attempt is allowed succeeded normally. Root cause, traced in the quiz component's own source: the function that checks the attempt limit returns a human-readable sentence (for example ``You have used all 2 allowed attempts for this quiz.'') in its \tcode{message} field once the limit is reached, and the component's response to that check wrote \tcode{reason: attemptCheck.message || 'max_attempts'} -- since the message string itself is truthy, \tcode{reason} was set to the full sentence rather than to the category key \tcode{'max_attempts'} the rendering logic below actually switches on. The block that is supposed to render for \tcode{reason === 'max_attempts'} therefore never matched, and none of the other blocks matched a full sentence either, so the card rendered with nothing inside it. Fixed by keeping \tcode{reason} fixed at the category key and carrying the human-readable sentence in a separate field, displayed as the description under the ``Maximum attempts reached'' heading. Verified live: the same account, the same exhausted quiz, now shows the heading, a warning icon, and the exact sentence explaining that all attempts have been used; the automated check for this scenario was updated to assert on that visible text and passes cleanly.
...
The findings in Table~\ref{tab:wcag_reviewer} are final as reported, and several are now backed by a real rendered browser and, in three cases, a real fix, rather than code reading alone. \textbf{1.2.1 Video-only} is a live Fail (2 of 7 published lessons with video have no transcript, against the auditor's own rule). \textbf{2.2.1 Timing Adjustable} is a Fail that contradicts Table~\ref{tab:wcag_map}'s existing claim (confirmed in code: a baseline, no-preset learner's quiz auto-submits with no warning when the timer reaches zero, and only a learner with a preset active gets the Extend/Submit-now prompt the table describes). \textbf{1.4.8 Visual Presentation} was a confirmed Fail -- the Dyslexia preset's own theme was being silently overridden by an unrelated legacy setting -- and is now fixed and verified live (Defect 3, Section~\ref{sec:test-defects}). \textbf{2.5.8 Target Size} was likewise a confirmed Fail, not on any control's own dimensions but on the Autism preset's checklist sidebar intercepting clicks meant for the button beneath it, and is now fixed and verified live (Defect 2). \textbf{4.1.2 Name, Role, Value} recorded a lesson-completion control that was completely unreachable under the ADHD preset; that gap is now closed and reverified (Defect 6). \textbf{2.2.4 Interruptions} remains Partial: the automated suite's finding that distraction-free mode removes the sidebar across the whole application, not only inside a lesson, still stands unfixed, and the heuristic pass's own read of whether the reduced-chrome view feels calmer is deliberately left as a subjective judgement for a human tester (Table~\ref{tab:manual_spotcheck}, MSC-04) rather than asserted from a structural read alone. \textbf{2.3.3 Animation} is a new, code-confirmed Partial: the CSS-level animation suppression is comprehensive for native transitions but has no effect on the Framer Motion celebration animation, which still runs on its hardcoded duration regardless of the setting -- a real gap, left unfixed as it was out of scope for this testing pass. \textbf{TODO:} the remaining rows marked TODO in Table~\ref{tab:wcag_reviewer} still need sign-off from a human reviewer, since they call for a screen reader, an assistive-technology session, or a subjective read of an actual sensory experience that a scripted browser session cannot supply.
...
Three of this chapter's five evidence classes are now complete: the automated end-to-end suite, the load and stress tests, and the WCAG 2.2 expert review's objective rows. Run against the local development stack (Section~\ref{sec:test-env}), not the deployed application, the automated suite's final, post-fix run passes eighteen of its nineteen scenarios cleanly, with the nineteenth (ADMIN-01) passing reliably but flaky on a documented local-server cause rather than an application defect, and none failing. Getting there found seven genuine defects -- four surfaced by the automated suite, one by the load tests, and two by the manual/heuristic inspection -- and all seven were fixed in the codebase and reverified the same day, written up in full with root cause and verification in Section~\ref{sec:test-defects}; nothing already passing regressed in the process. The load tests found the same headroom the automated suite's functional results imply: all three k6 scenarios and a targeted concurrency regression test held their latency thresholds with an effective 0\% error rate at a realistic worst-case concurrency for this platform's scale, after the one real concurrency defect the first run surfaced (Defect 4) was fixed. Assembling the WCAG review turned up a Fail that contradicts an existing claim elsewhere in this report (Table~\ref{tab:wcag_map}, criterion 2.2.1) and needs a decision on whether that claim should be corrected, alongside the three findings now fixed and the two now-completed heuristic checks (2.2.4, 2.3.3) described above. The manual spot-check matrix and user acceptance testing remain pending, exactly as Table~\ref{tab:manual_spotcheck} and Section~\ref{sec:test-results}'s UAT results state; once both are supplied, this section will be rewritten to state what all five classes together show, including where they disagree with each other or with earlier chapters.
