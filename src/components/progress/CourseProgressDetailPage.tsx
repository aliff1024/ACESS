    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 md:py-10 space-y-8 readable-content">
      ...
      {isCompleted && (
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-amber-400 via-yellow-500 to-orange-500 text-white shadow-xl p-8 md:p-10 border border-yellow-300 flex flex-col md:flex-row items-center justify-between gap-8 transform hover:scale-[1.01] transition-transform">
            ...
            <div>
              ...
              <h3 className="text-3xl font-extrabold tracking-tight mb-1 drop-shadow-sm">
                {course.certificate_id 
                  ? t('courseProgress.certificateEarned') 
                  : (course.certificate_enabled && !course.cert_eligible) 
                    ? 'Quiz Improvements Needed'
                    : t('courseProgress.incredibleWork')}
              </h3>
              <p className="text-yellow-50 text-lg font-medium opacity-90">
                {course.certificate_id 
                  ? t('courseProgress.certificateEarnedDesc') 
                  : course.certificate_enabled 
                    ? (!course.cert_eligible 
                        ? (course.cert_eligibility_reason || 'Please pass all quizzes to claim your certificate.')
          ...
          <div className="flex flex-col sm:flex-row gap-3 z-10 w-full md:w-auto shrink-0 items-center">
            {/* System Certificate Button */}
            {course.certificate_id ? (
              <Button
                onClick={() => window.location.href = `/learner/certificates?id=${course.certificate_id}`}
                className="bg-white text-yellow-600 hover:bg-yellow-50 shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all px-6 py-6 text-base font-bold rounded-2xl w-full sm:w-auto whitespace-nowrap border-0 flex items-center justify-center"
              >
                <Trophy className="w-4 h-4 mr-2 text-yellow-600" /> {t('courseProgress.viewCert') || 'View Certificate'}
              </Button>
            ) : (course.certificate_enabled && course.cert_eligible) ? (
              <Button
              ...
              <div className="text-xs text-yellow-100 bg-black/10 backdrop-blur-sm px-4 py-2.5 rounded-xl border border-white/20 font-medium">
                Certificate Status: <span className="font-bold text-white">Quizzes Pending</span>
              </div>
            ) : null}

            {/* Educator Certificate Button/Badge */}
            {course.custom_cert_pdf_url ? (
              <Button
                onClick={() => window.open(course.custom_cert_pdf_url, '_blank')}
                className="bg-purple-600 hover:bg-purple-700 text-white shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all px-6 py-6 text-base font-bold rounded-2xl w-full sm:w-auto whitespace-nowrap border-0 flex items-center justify-center"
              >
                <GraduationCap className="w-4 h-4 mr-2" /> {t('courseProgress.viewEducatorCert')}
              </Button>
            ) : course.allow_custom_certs ? (
              <div className="text-xs text-yellow-100 bg-black/10 backdrop-blur-sm px-4 py-2.5 rounded-xl border border-white/20 font-medium">
                {t('courseProgress.educatorCertPendingShort')}
              </div>
            ) : null}
          </div>
        </div>
      ...
      {/* Quiz Improvements Needed Card */}
      {isCompleted && course.certificate_enabled && !course.certificate_id && !course.cert_eligible && course.quizzes_need_improvement && course.quizzes_need_improvement.length > 0 && (
        <Card className="p-6 border-amber-200 bg-amber-50/50 rounded-3xl border shadow-sm">
          <div className="flex items-start gap-4">
            <div className="w-12 h-12 bg-amber-100 rounded-2xl flex items-center justify-center flex-shrink-0 shadow-sm border border-amber-200">
              <Clock className="w-6 h-6 text-amber-600" />
            </div>
            <div className="flex-1">
              <h4 className="text-lg font-bold text-amber-900 mb-1">Quizzes Needing Improvement</h4>
              <p className="text-sm text-amber-700 mb-4 leading-relaxed">
                You have completed 100% of the lessons, but you must pass the quizzes below to reach the required passing threshold to unlock your certificate.
              </p>
              <div className="bg-white rounded-2xl border border-amber-100 overflow-hidden divide-y divide-gray-100">
                {course.quizzes_need_improvement.map((q) => (
                  <div key={q.quizId} className="p-4 flex items-center justify-between gap-4 hover:bg-gray-50/50 transition-colors">
                    <div>
                      <p className="text-sm font-semibold text-gray-900">{q.lessonTitle}</p>
                      <p className="text-xs text-gray-500 mt-0.5">{q.quizTitle}</p>
                    </div>
                    <Button 
                      size="sm" 
